...
async def async_setup_entry(
    hass: HomeAssistant, entry: LKConfigEntry, async_add_entities: AddEntitiesCallback
) -> None:
    ...
    if coordinator.data and "devices" in coordinator.data:
        for device in coordinator.data["devices"]:
            ...
            if (
                ...
                and device_mac
                and device_mac not in processed_devices
            ):
                ...
                if (
                    ...
                    and device["measurement"].get("desiredTemperature") is not None
                ):
                    entities.append(
                        LKThermostat(
    ...
    if coordinator.data.get("hub_data"):
        for _hub_id, hub_data in coordinator.data["hub_data"].items():
            if isinstance(hub_data, dict) and "devices" in hub_data:
                for device in hub_data["devices"]:
                    ...
                    if (
                        ...
                        if (
                            "measurement" in device
                            and device["measurement"].get("desiredTemperature")
                            is not None
                        ):
                            entities.append(
                                LKThermostat(
...
class LKThermostat(CoordinatorEntity, ClimateEntity):
    ...
    def __init__(
        ...
        if parent_identity:
            ...
            _LOGGER.debug(
                "Thermostat %s connected via hub %s", device_identity, parent_identity
            )
        else:
            _LOGGER.debug(
                "No parent identity found for thermostat %s, will appear as standalone device",
                device_identity,
            )
    ...
    async def _set_temperature(self, temperature: float) -> None:
        ...
        _LOGGER.debug(
            "Setting temperature for %s to %s°C", self._device_identity, temperature
        )
        ...
        try:
            ...
            result = await self.coordinator.set_thermostat_temperature(
                self._device_identity, api_temp
            )
    ...
    async def async_set_preset_mode(self, preset_mode: str) -> None:
        """Set preset mode."""
        _LOGGER.debug(
            "Setting preset mode for %s to %s", self._device_identity, preset_mode
        )
